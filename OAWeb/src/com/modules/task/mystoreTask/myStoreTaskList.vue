            ...
            <div class="TaskList"v-for="(item, index) in recievetaskList" :key="index" @click="jumpDetail(item.id,1)">
              ...
              <div class="header_box bgcolor_white pt20 pl20 pm10 itemBoxShadow">
                ...
                <div class="card-padding">
                  ...
                  <div class="BtnBottom titleGray aptl">
                    <span v-if="item.isCollect==0"><img src="@/assets/store.png" alt="" @click.stop="reciveChangeStore(item.id,item.isCollect)" style="width: 18px;height:17px;margin-right: 20px;"></span>
                    <span v-else><img src="@/assets/chosedStore.png" alt="" @click.stop="reciveChangeStore(item.id,item.isCollect)" style="width: 18px;height:17px;margin-right: 20px;"></span>
                    <span v-if="item.isTop==0"><img src="@/assets/toTop.png" alt="" @click.stop="reciveToTop(item.id,item.isTop)" style="width: 18px;height:17px;"></span>
                    <span v-else><img src="@/assets/chosedToTop.png" alt="" @click.stop="reciveToTop(item.id,item.isTop)" style="width: 18px;height:17px;"></span>
                  </div>
                </div>
              </div>
            </div>
...
//        var diffheight=window.screen.height-topHeight-100;
        var diffheight=document.body.clientHeight-topHeight;
...
//      创建任务
      ...
      recieveTast(){
        ...
        util.getList("taskInfo/getToMeCollectList",{//我接收的
          'userId':this.userId,
          'taskName':taskName
      ...
      sendTask(){
        let taskName = this.value;//搜索框条件
        util.getList("taskInfo/getFromMeCollectList",{//我发布的
          'userId':this.userId,
          'taskName':taskName
        }).then(response => {//我发布的任务
          console.log(response.data.result);
          ...
          for(let k in this.taskList){
            ...
            this.taskList[k].lastReportDate=this.checkDate(new Date(), this.taskList[k].lastReportDate.replace(/\-/g, "/"));//反馈时间
          }
        });
      },
      showMenu(){
...
//      this.list2.push("我发布的")
//      this.list2.push("我接收的");
...
<style lang="less">
  ...
  #myStoreTaskHeader .vux-tab-wrap{
    overflow: hidden!important;
  ...
  #myStoreTaskHeader .vux-tab{
    background-color: #fff!important;
    height:37px!important;
  }
  #myStoreTaskHeader .vux-tab-container{
    height: 52px !important;
    background-color: #fff!important;
    position: inherit;
    /*top:2px;*/
  }
  #myStoreTaskHeader .vux-tab .vux-tab-item{
  ...
  #myStoreTaskHeader .vux-tab-ink-bar.vux-tab-ink-bar-transition-backward,.vux-tab-ink-bar.vux-tab-ink-bar-transition-forward{
     width:12% !important;
  ...
  #myStoreTaskHeader .vux-header{
    background: #fff!important;
    color: #fff;
  }
  #myStoreTaskHeader .vux-header .vux-header-left .left-arrow:before{
    border-color: #323232!important;
  }
  #myStoreTaskHeader .vux-header .vux-header-title{
    color: #323232!important;
  }
  #myStoreTaskHeader .weui-search-bar{
    top:5px;
 ...
 .vux-search-fixed{
  ...
  #myStoreTaskHeader .weui-search-bar.weui-search-bar_focusing .weui-search-bar__cancel-btn{
    display: none !important;
    display: none !important;
  }
  #myStoreTaskHeader .weui-search-bar__box .weui-search-bar__input{
    height:2em;
  }
  #myStoreTaskHeader .weui-search-bar__box .weui-icon-search, #myStoreTaskHeader .weui-search-bar__box .weui-icon-clear{
    line-height:39px;
  }
  #myStoreTaskHeader .vux-x-icon{
    bottom:65px;
  ...
  #myStoreTaskHeader .vux-header .vux-header-right .vux-header-more:after{
