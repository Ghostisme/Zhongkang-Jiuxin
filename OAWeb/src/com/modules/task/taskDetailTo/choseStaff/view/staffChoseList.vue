      ...
      >
        ...
        >
          ...
          <span
            ...
            :class="[itemList.choseHeart?'clickTrue':'clickFalse',{heartactive:itemList.isDisabled},{heartNone:heartNone}]"
            v-model="mainPeople"
  ...
  methods: {
    ...
    btnOk() {
      ...
      if (storejs.get("detailToChosed") == "2") {
        //2是执行人员
        storejs.set(
          "detailtask_selectPeople",
          JSON.stringify(this.implementUserList)
        );
      } else if (storejs.get("detailToChosed") == "1") {
        //是围观人员
        storejs.set(
          "detailonlook_selectPeople",
          JSON.stringify(this.implementUserList)
        );
      }
      console.log(JSON.stringify(this.implementUserList));
      util
        .getList("taskUser/saveTaskUser", {
          userList: JSON.stringify(this.implementUserList),
          userType: Number(storejs.get("detailToChosed")),
          taskId: taskId
        })
        .then(response => {
//          this.$router.push({ path: "/taskDetails" });
  ...
  created() {
    ...
    //    if (fkDeptId) {
    //      //获得用户数据
    //      util.get("taskUser/getEditUserList?fkDeptId=" + fkDeptId).then(response => {
    ...
    //          }
    //        }
    //        if(storejs.get("detailToChosed")=="2"){//从详情的执行人员跳转过来的
    //           if(storejs.get("detailonlook_selectPeople")){
    //             this.disabledValue=JSON.parse(storejs.get("detailonlook_selectPeople"));//获取围观人员进行互斥选择
    //           }
    //          if(storejs.get("detailtask_selectPeople")){
    //            this.canChoseValue=JSON.parse(storejs.get("detailtask_selectPeople"));//获取可选择执行人员
    //          }
    //        }else if(storejs.get("detailToChosed")=="1"){//从详情的围观人员跳转过来的
    //          if(storejs.get("detailtask_selectPeople")){
    //            this.disabledValue=JSON.parse(storejs.get("detailtask_selectPeople"));//获取执行人员互斥选择
    //          }
    //          if(storejs.get("detailonlook_selectPeople")){
    //            this.canChoseValue=JSON.parse(storejs.get("detailonlook_selectPeople"));//获取可选择围观人员
    //          }
    //        }
    //        console.log(storejs.get("detailtask_selectPeople"));
    ////        不可选人员添加置灰属性
    ...
    //选择的部门id报错情况下获得用户数据
    util
      .getList("/taskUser/getEditUserList", {
        fkDeptId: fkDeptId,
        name: "",
        taskId: taskId
      })
      .then(response => {
        ...
        if (storejs.get("detailToChosed") == "2") {
          //从详情的执行人员跳转过来的
          //        this.$router.push("{title:'选择执行人员'}")
          if (storejs.get("detailonlook_selectPeople")) {
            this.disabledValue = JSON.parse(
              storejs.get("detailonlook_selectPeople")
            ); //获取围观人员进行互斥选择
          }
          if (storejs.get("detailtask_selectPeople")) {
            this.canChoseValue = JSON.parse(
              storejs.get("detailtask_selectPeople")
            ); //获取可选择执行人员
          }
        } else if (storejs.get("detailToChosed") == "1") {
          //从详情的围观人员跳转过来的
          //        this.$router.push("{title:'选择围观人员'}")
          if (storejs.get("detailtask_selectPeople")) {
            this.disabledValue = JSON.parse(
              storejs.get("detailtask_selectPeople")
            ); //获取执行人员互斥选择
