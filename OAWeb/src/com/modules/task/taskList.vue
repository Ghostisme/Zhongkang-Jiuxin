...
<template>
  <div id="taskHeader">
    ...
    <!--</x-header>-->
    <tab :line-width=2 active-color='#fff' v-model="index" @click.native="showMenus=false" :class="{sendTabs:leaderCode==0}" >
    ...
    <swiper v-model="index" :show-dots="false" ref="cateSwiper" @on-get-height="updataHeight" :class="{recieveSwiperTrans:leaderCode==0}"@on-index-change="AddIcon(index)">
      <swiper-item id="swiper1"  :class="{sendSwiperDisplay:leaderCode==0}">
      ...
      <div class="addTaskBtnBox"style='' :class="{AddIconDisplays:AddIconDisplay,sendSwiperDisplay:leaderCode==0}">
        <!--<x-icon type="ios-plus" size="30" @click="btCreateTask"></x-icon>-->
        <!--<div class="btnAdd" @click="btCreateTask">+</div>-->
...
//        this.$router.push({ path: '/taskDetails'})
          this.$router.push({ path: '/taskProgress'})
      },
      passTabItem(index){
        if(index==0){
          this.index=1;
...
//        this.$vux.toast.text(this.index,'top');
        if(this.index==1){
          this.recieveTast();
        }else{
          this.sendTask();
...
//        alert(index);
        if(index==1){
          this.AddIconDisplay=false;
        }else{
          this.AddIconDisplay=true;
        }
      ...
      myreload(){
        ...
        if (storejs.get("myload") ==undefined) {

          storejs.set("myload",1);
//          this.$vux.toast.text(storejs.get("myload"), 'middle');
          setTimeout(function(){
            that.reload();
            }, 0);
        }else{
          storejs.remove("myload");
        }
      },
    },
    created(){
...
<style lang="less">
  ...
  .vux-tab-ink-bar.vux-tab-ink-bar-transition-backward,.vux-tab-ink-bar.vux-tab-ink-bar-transition-forward{
     width:12% !important;
