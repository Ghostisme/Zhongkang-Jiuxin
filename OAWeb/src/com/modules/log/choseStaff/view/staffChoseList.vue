...
export default {
  ...
  methods: {
    ...
    btnOk() {
      ...
      const howpageId = localStorage.getItem("howpageId");
      this.$router.go(-3);
      if (howpageId == 1) {
      	this.$router.replace({ path:"/log/comment" });
      	storejs.set("commentChoosePeople", JSON.stringify(this.implementUserList));
      } else{
      	this.$router.replace({ path:"/log/addLog" });
      	storejs.set("logChoosePeople", JSON.stringify(this.implementUserList));
      }
  ...
  created() {
    ...
    if (fkDeptId) {
      ...
      util.get("sys/user/list?fkDeptId=" + fkDeptId).then(response => {
        ...
        const howpageId = localStorage.getItem("howpageId");
        if (howpageId == 1) {
          if(storejs.get("commentChoosePeople")){
            this.canChoseValue = JSON.parse(storejs.get("commentChoosePeople"));
          }else{
            this.canChoseValue = [];
          }
        } else{
          if(storejs.get("logChoosePeople")){
            this.canChoseValue = JSON.parse(storejs.get("logChoosePeople"));
          }else{
            this.canChoseValue = [];
          }
        }
