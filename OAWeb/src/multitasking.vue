...
//      创建任务
      ...
      recieveTast(){
        ...
        util.getList("taskInfo/getToMeList",{//我接收的
          ...
          for(let k in this.recievetaskList){
            this.recievetaskList[k].lastCommentDate=util.formatDate(new Date(this.recievetaskList[k].lastCommentDate),"yyyy-MM-dd hh:mm:ss");
            this.recievetaskList[k].createDate=util.formatDate(new Date(this.recievetaskList[k].createDate),"yyyy-MM-dd");//创建时间
            this.recievetaskList[k].planFinishTime=util.formatDate(new Date(this.recievetaskList[k].planFinishTime),"yyyy-MM-dd");//结束时间
            this.recievetaskList[k].lastReportDate=this.checkDate(new Date(), new Date(this.recievetaskList[k].lastReportDate));//反馈时间
          }
        })
          .catch(error => {
      ...
      sendTask(){
        let taskName = this.value;//搜索框条件
        util.getList("taskInfo/getFromMeList",{//我发布的
          'userId':this.userId,
          'taskName':taskName
        }).then(response => {//我发布的任务
          console.log(response.data.result);
          ...
          for(let k in this.taskList){
            this.taskList[k].lastCommentDate=util.formatDate(new Date(this.taskList[k].lastCommentDate),"yyyy-MM-dd hh:mm:ss");
            this.taskList[k].createDate=util.formatDate(new Date(this.taskList[k].createDate),"yyyy-MM-dd");//创建时间
            this.taskList[k].planFinishTime=util.formatDate(new Date(this.taskList[k].planFinishTime),"yyyy-MM-dd");//结束时间
            this.taskList[k].lastReportDate=this.checkDate(new Date(), this.taskList[k].lastReportDate);//反馈时间
          }
        });
      },
      showMenu(){
...
<style lang="less">
  ...
  #taskHeader .vux-tab-ink-bar.vux-tab-ink-bar-transition-backward,.vux-tab-ink-bar.vux-tab-ink-bar-transition-forward{
    width:12% !important;
  ...
  #taskHeader .weui-search-bar.weui-search-bar_focusing .weui-search-bar__cancel-btn{
    display: none !important;
  }
  #taskHeader .weui-search-bar__box .weui-search-bar__input{
    height:2em;
  }
  #taskHeader .weui-search-bar__box .weui-icon-search, #taskHeader .weui-search-bar__box .weui-icon-clear{
    line-height:39px;
  }
  #taskHeader .vux-x-icon{
    bottom:65px;
