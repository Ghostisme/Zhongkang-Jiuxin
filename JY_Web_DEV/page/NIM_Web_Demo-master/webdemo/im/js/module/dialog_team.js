...
//      this.isOpen = true;
        ...
        $dialog.on('click', '.sureVideo', function (e) {//以前是.j-confirm
            e.preventDefault();
...
//          	新加功能
			console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",$addUserUl.find("li"));
			var $addUserLI=$addUserUl.find("li");
			for(var k=0;k<$addUserLI.length;k++){
				$($addUserLI[k]).click();
			}
//			新加功能结束
            /** 禁言UI，放开按钮 */
...
//              dialog.find('.j-confirm').removeClass('disabled');
                dialog.find('.sureVideo').removeClass('disabled');//新加
            }
...
//          that.$dialog.find('.j-confirm').toggleClass('disabled', addedNum <= 0)
			that.$dialog.find('.sureVideo').toggleClass('disabled', addedNum <= 0)//新加
        }
    },
    /** 关闭弹框 */
    close: function () {
