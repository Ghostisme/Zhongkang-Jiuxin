...
//				id: id
			...
			ajaxGet("/operate/assessment/getOperateProject", data, function(data){
				console.log(data);
				if(data.status == "0") {
//					this.dataList = data.resultMap.operateAssessment;
//					let dataList = data.resultMap.operateProjectInfo;
					let resDataList = data.resultMap.operateProjectInfo;
//					console.log(this.dataList)
					//基础信息
					that.projectName = resDataList.projectName;
					that.outName = resDataList.name;
					that.address = resDataList.address;
					that.outPhone = resDataList.phone;
					that.hospitalName = resDataList.hospitalName;
//					let resDataList = data.resultMap.list;
					let dataList = data.resultMap.list;
					
					for(let j = 0; j < dataList.length; j++){
						let outerInArr = dataList[j].children;
						let outerOutArr = dataList[j].childrend2;
						let hospitalInnerList = [];
						let hospitalOuterList = [];
						//处理院内院外的数据
						...
						for(let i = 0; i < outerOutArr.length; i++){
							...
							hospitalOuterList.push(obj);
						}
						
						let obj1 = {
							time: dataList[j].time,
							hospitalInnerList: hospitalInnerList,
							hospitalOuterList: hospitalOuterList
						}
						that.dataList.push(obj1);
					}
...
//			status--;
	...
	mounted() {
		let name = sessionStorage.getItem("dataListId");
		let userId = sessionStorage.getItem("dataListUserId");
		this.id = name;
		this.getData(name, userId);
		//查看修改
