...
function newWebSocket() {
    ...
    if ('WebSocket' in window) {
        ...
        var userSocket = sessionStorage.getItem('userSocket');
        var token=sessionStorage.getItem('token');
        if (user == null) {
            location.href = 'login.html';
        }
        console.log('useruseruser::::', user);
//      var wsUrl = "wss://cloud.zk9y.com/wsApi/websocket/" + user.userSocket;
        var wsUrl = "ws://cloud-dev.zk9y.com:9000/wsApi/websocket/" + userSocket;//家里测试地址
//		var wsUrl = "ws://lin.vipgz2.idcfengye.com/websocket/" + userSocket;//在家宏剑地址
...
//      msg = msg.split(',')[0];
        msg = msg.split('?');
        //0:在其他设备登录1：开始会诊 2：申请人结束会诊 3：会诊方结束会诊 4.收到会诊申请 5：预约检查所有消息 6.拒绝会诊 7.通过会诊  8.取消会诊
        if (msg[0] == "2") {
            // alert("对方拒绝了您的视频邀请");
...
//              console.log(msg[2].split('=')[1]);
    ...
    var accid=JSON.parse(sessionStorage.getItem('user')).accid
	function getToken() {
		ajaxGet('/sys/user/refreshToken',{accid:accid},function(data) {
						console.log(data);
...
// 连接发生错误，连接错误时会继续尝试发起连接（尝试5次）
    websocket.onerror = function () {
        ...
        if (websocket_connected_count <= 2) {
