...
function getDrugsInfo() {
	$.ajax({
		...
		success: function(data) {
			...
			imgListLength = data.resultMap.commodityInfoImgList.length;
...
function uploadFiles(formData) {
	$.ajax({
		...
		success: function(data) {
			...
			if(data.status=="1"){
				swal("提交成功！");
				closeNavTabAndRefreshIframe("page/changeProductDetail/changeProductDetail.html", "page/drugList/drugList.html")
			}else{
				swal(data.message);
			}
