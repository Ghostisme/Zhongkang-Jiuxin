...
$(document).on("click", ".nameAndProv", function() {
	...
	getFare($(this).find('.recieveName').next('span').text());
});
		...
		success: function(data) {
			...
			if(data.status == 1) {
				if(getParam('detailToOrder')) { //从商品详情立即下单
					...
					khtml += '<input autocomplete="off" type="text" class="numItxt" value="' + buyQuickNum + '" class="changeQuantity" minnum="1">';
					...
					$(".productPrice").text(data.resultMap.totalPrice.toFixed(2)); //商品金额
				...
				} else if(getParam('packagedDetailToOrder')){//从套餐详情跳转
						var drugInfo = data.resultMap.mealInfo;
						var mealList = data.resultMap.mealInfo.mealList;//是否是套餐商品
						//					console.log(drugInfo);
						var khtml = '';
						if(mealList.length>0){
							
							khtml +='<div class="packageSimpleOrder">'
							khtml +='<div class="packageTitle">';
							khtml +='<span class="lf"><span style="font-weight:bold;">【套装】</span>'+drugInfo.mealName+'</span>';
							khtml += '<div class="packageCurrentDrugId" style="display:none;">' + drugInfo.id + '</div>';
//							khtml += '<div class="cid" style="display:none;">' + data.resultMap.drugInfoList[i].cid + '</div>';
//							if(data.resultMap.drugInfoList[i].sign != "" && data.resultMap.drugInfoList[i].sign != null) {
//								khtml += '<div class="sign" style="display:none;">' + data.resultMap.drugInfoList[i].sign + '</div>';
//							}
							khtml += '<div class="detailStyle rt changeQuantityBox packageRed">套餐价:<span>¥<span class="packageTopPrice">'+data.resultMap.mealPrice+'</span></span></div>';
							khtml += '<div class="detailStyle rt changeQuantityBox" style="margin-right:6px;margin-top:10px;">';
							khtml += '<div class="cartNumBox"style="display:block;">';
							khtml += '<a href="javascript:void(0);" clstag="clickcart|keycount|xincart|cart_num_down" class="decrement disabled lf packageDec">-</a>';
							khtml += '<input autocomplete="off" type="text" class="numItxt lf packageNumItxt changeQuantity" value="' + buyQuickNum  + '" minnum="1">';
							khtml += '<a href="javascript:void(0);" clstag="clickcart|keycount|xincart|cart_num_up" class="increment rt packageInc">+</a>';
							khtml += '</div>';
							khtml += '</div>';
//							khtml += '<div class="detailStyle rt changeQuantityBox"style="margin-top:0px;">';
//							khtml += '<span class="thisNum"style="display:block;margin-right:8px;">' + getParam("number") + '</span>';
//							khtml +='</div>'
							khtml +='</div>'
							
							for(var k = 0; k < mealList.length; k++){
								khtml += '<div class="simpleOrder"style="border-top:0px;">';
								khtml += '<div class="mealid" style="display:none;">' + mealList[k].id + '</div>';
								khtml += '<div class="simpleCheckbox lf detailStyle">';
								khtml += '</div>';
								khtml += '<div class="productImg lf detailStyle">';
								if(mealList[k].fileUrl){
									khtml += '<img src="'+mealList[k].fileUrl+'" alt="" />';
								}else{
									khtml += '<img src="../../images/shopCart/productImg1.png" alt="" />';
								}
								khtml += '</div>';
								khtml += '<div class="nameParent lf detailStyle">';
								khtml += '<div style="line-height:1.6;" class="productName">' + mealList[k].fullName + '</div>';
								khtml += '<div style="line-height:1.3;" class="specs">' + mealList[k].specs + '</div>';
								khtml += '</div>';
								khtml += '<div class="detailStyle lf priceBox">';
								khtml += '<span>¥</span>';
								khtml += '<span class="price">' + mealList[k].presentPrice + '</span>';
								if(mealList[k].drugNum!=1){
								khtml += '<span class="RMBmoney_unit">/' +mealList[k].drugNum+ mealList[k].unit + '</span>';
								}else{
									khtml += '<span class="RMBmoney_unit">/'  +mealList[k].unit + '</span>';
								}
								
	//							kHtml += '<div class="fullMinus">满<span class="full">600</span>减<span class="minus">200</span></div>';			
								khtml += '<div class="fullMinus">';
								khtml += '<span class="drugprice">¥' + mealList[k].drugPrice +'/'+ mealList[k].unit  + '</span>';
								khtml += '</div>';
								khtml += '</div>';
								khtml += '<div class="detailStyle lf changeQuantityBox">';
		//						if(getParam('orderId')) {
		...
		//							khtml += '</div>';
		//						} else {
									khtml += '<span class="thisNum"style="display:block;">' + getParam("number") + '</span>';
		//						}
								khtml += '</div>';
								khtml += '<div class="detailStyle lf subtotalBox">￥';
								khtml += '<span class="subtotal">' + mealList[k].commodityCountPirce.toFixed(2) + '</span>';
								khtml += '</div>';
								khtml += '</div>';
							}
						...
						$(".num").text(data.resultMap.goodNum); //商品总量	
						//				$(".RMB_total").text(data.resultMap.Subtotal); //商品小计
						$(".productPrice").text(data.resultMap.mealPrice.toFixed(2)); //商品金额
						$(".freeMoney").text(data.resultMap.mealInfo.discountPrice); //优惠金额
						$(".freightPrice").text(data.resultMap.express); //运费
						//				$(".shop_count_all").text(data.resultMap.goodNum); //结算条总数
						$(".TotalPrice").text(data.resultMap.amountPayable.toFixed(2)); //结算条应付金额
						goodsNum = drugInfo.number; //商品数传值
					...
					for(var i = 0; i < data.resultMap.list.length; i++) {
						...
						var mealList = data.resultMap.list[i].drugInfo.mealList;//是否是套餐商品
						//					console.log(drugInfo);
						var khtml = '';
						if(mealList){
...
//							}
							...
							khtml += '<div class="detailStyle rt changeQuantityBox packageRed">套餐价:<span>¥<span class="packageTopPrice">'+drugInfo.totalPrice+'</span></span></div>';
//							khtml += '<div class="detailStyle rt changeQuantityBox" style="margin-right:6px;margin-top:10px;">';
//							khtml += '<a href="javascript:void(0);" clstag="clickcart|keycount|xincart|cart_num_down" class="decrement disabled lf packageDec">-</a>';
//							khtml += '<input autocomplete="off" type="text" class="numItxt packageNumItxt" value="' + data.resultMap.list[i].drugNum + '" class="changeQuantity packageChangeQuantity" minnum="1">';
//							khtml += '<a href="javascript:void(0);" clstag="clickcart|keycount|xincart|cart_num_up" class="increment rt packageInc">+</a>';
//							khtml += '</div>';
							khtml += '<div class="detailStyle rt changeQuantityBox"style="margin-top:0px;">';
							khtml += '<span class="thisNum"style="display:block;margin-right:8px;">' + data.resultMap.list[i].drugNum + '</span>';
							khtml +='</div>'
							khtml +='</div>'
	...
	//						}
					...
					$(".freeMoney").text(data.resultMap.discountPrice.toFixed(2)); //优惠金额
...
//				数量change
				$(document).on("change",".numItxt",function() { //更改数量
...
//			}
			else if(!(($(".writetaxtTel").val().length == 11||$(".writetaxtTel").val().length == 7||$(".writetaxtTel").val().length == 8||$(".writetaxtTel").val().length == 13||$(".writetaxtTel").val().length == 12) && /^((0\d{2,3}-\d{7,8})|(1[3584]\d{9}))$/.test($(".writetaxtTel").val()))){
				$(".messageBG").css("display", "block");
				$(".messageWarnsText").text("请输入正确的注册电话！");
...
$(".submit-btn").click(function() {
	if($(".chosedNameAndProv").length == 0) {
		...
		var orderUser = $(".chosedNameAndProv .recieveName").text(); //用户姓名 
