<template>
  ...
  <div  class="app-container documentation-container" style="width:100%;height:800px;padding-top:0px;margin-top:10px;">
    ...
    <!--<input type="file" id="insertImg" name="image" class="getImgUrl_file" @change="insertFile($event)">-->
    <!--<div v-on:click="jumpToOcx()"id="jumpToOcxBtn" style="background: linear-gradient(180deg,#6e97af,#5a91af,#3d8ebc,#3c90bf);color: #fff;width:150px;height:25px;line-height:25px;text-align: center;">编写电子病历</div>-->
...
<script>
  import { jumpOcx } from '@/utils/jumpCommon'
  export default {
    name: 'rdsidoc',
    data(){
      return {
...
//      this.newFile()
      this.hisJspUrl="http://192.31.10.85:8081/215his/PharmacyResident.jsp?HOSP_AREA="+this.userInfo.hosArea+"&DEPT_CODE="+this.userInfo.deptCode+"&USER_ID="+this.userInfo.userId;
    },
    methods: {
      //菜单开始
      ...
      jumpToOcx:function(){
//        this.$router.push('/jumpmiddlePage/index')
//        jumpOcx(this);
...
//        ocxobj.FileSaveAs('D:\\住院病历.emr',1,0);
      ...
      newFile() {
        const ocxobj = document.getElementById('preview');
        ocxobj.universalboolfunction("O9481C-NNR5HA-EE0E9E-EO22PR","",98,25130824);
        ocxobj.FileNew();
